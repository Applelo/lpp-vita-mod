...
static int lua_touchpad(lua_State *L)
{
		...
		SceTouchData touch;
		int report;
		int panel= luaL_checknumber(L, 1);
		if (argc == 1) report = 1;
		else report = luaL_checknumber(L, 2);
		
		if (report>6 && report<0 && panel==0) return luaL_error(L, "touch report is limited by 6 on front.");
		if (report>4 && report<0 && panel==1) return luaL_error(L, "touch report is limited by 4 on back.");
		
		sceTouchPeek(luaL_checknumber(L, 1), &touch,1);
		
		lua_pushnumber(L, lerp(touch.report[report-1].x, 1919, 960));
		lua_pushnumber(L, lerp(touch.report[report-1].y, 1087, 544));
...
void luaControls_init(lua_State *L) {
	...
	VariableRegister(L,SCE_TOUCH_PORT_FRONT);
	VariableRegister(L,SCE_TOUCH_PORT_BACK);
}